...
timeout 30s ../cegtools/deploy/cegLog cegLogBuild $PATH_SO /etc/ssl/certs $LOGHOST $LOGPORT $TOKEN $GUID
RETVAL="$?"
...
timeout 30s ../cegtools/deploy/cegInstrument cegRecordBuild $PATH_SO /etc/ssl/certs $METHOST $METPORT $TOKEN $GUID
RETVAL="$?"
# NOTE RETVAL is an unsigned 7 bit value so will never be > 255
#echo "RETVAL is $RETVAL"
...
if [ $RETVAL -eq 0 ]; then
  echo "Metric sent."
    ...
    echo "cegInstrument command timed out."
    echo "Please review this machine's networking configuration"
    echo "firewall settings, and network egress routes and confirm"
    echo "that it can successfully reach $METHOST on $METPORT."
    echo "---------\n"
else
    echo "cegInstrument returned an error."
    echo "Please review the command output for more details."
    echo "---------\n"
fi
